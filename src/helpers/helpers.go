...
func CalculateFileHash(requestBody io.ReadCloser) (string, error) {
	hash := sha256.New()
	// Copy the contents of the request body onto the hash sum
	if _, err := io.Copy(hash, requestBody); err != nil {
		return "", fmt.Errorf("error while calculating the hash sum: %w", err)
	}
